/*
---------------------------------------------------------------------------------
Stored Procedure: Carga da Camada Silver
    1. Realiza o TRUNCATE das tabelas da camada Silver para limpeza.
    2. Executa as transformações e inserções (INSERT INTO ... SELECT) 
       da camada Bronze para a Silver.
...
BEGIN
    ...
    PRINT '===========================================================';
    PRINT 'Populando camada silver';
    PRINT '===========================================================';

    PRINT '--- Populando CRM (Cust, Prd, Sales) ---';
    ...
    SELECT 
        ...
        cst_create_date
    FROM (SELECT *, ROW_NUMBER() OVER (PARTITION BY cst_id ORDER BY cst_create_date DESC) AS flag FROM bronze.crm_cust_info) AS t
    WHERE flag = 1 AND cst_id IS NOT NULL;

    TRUNCATE TABLE silver.crm_prd_info;
    INSERT INTO silver.crm_prd_info (prd_id, cat_id, prd_key, prd_nm, prd_cost, prd_line, prd_start_dt, prd_end_dt)
    SELECT 
        prd_id, REPLACE(SUBSTRING(prd_key, 1, 5), '-', '_'), SUBSTRING(prd_key, 7, LEN(prd_key)), prd_nm, ISNULL(prd_cost, 0),
        CASE WHEN UPPER(TRIM(prd_line)) = 'M' THEN 'Mountain'
             WHEN UPPER(TRIM(prd_line)) = 'R' THEN 'Road'
             WHEN UPPER(TRIM(prd_line)) = 'S' THEN 'Other Sales'
             WHEN UPPER(TRIM(prd_line)) = 'T' THEN 'Touring' ELSE 'n/a' END,
        CAST(prd_start_dt AS DATE), LEAD(prd_start_dt) OVER (PARTITION BY prd_key ORDER BY prd_start_dt ASC) - 1
    FROM bronze.crm_prd_info;

    TRUNCATE TABLE silver.crm_sales_details;
    INSERT INTO silver.crm_sales_details (sls_order_num, sls_prd_key, sls_cust_id, sls_order_dt, sls_ship_dt, sls_due_dt, sls_quantity, sls_sales, sls_price)
    SELECT
        sls_order_num, sls_prd_key, sls_cust_id,
    ...
    PRINT '--- Populando ERP (Cust, Loc, Px_Cat) ---';
    ...
    SELECT
        ...
        CASE WHEN TRIM(cntry) = 'DE' THEN 'Germany'
             ...
             WHEN TRIM(cntry) = '' OR cntry IS NULL THEN 'n/a' ELSE TRIM(cntry) END
